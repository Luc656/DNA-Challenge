...
Takes a sequence as input and the number of consecutive bases before a run is defined (3 by default). The function uses a sliding-window approach, iterating through the bases, for each base (i) the function iterates again (j) until i != j if at any point j - i is greater then the run length then all bases are appended to a list. The function returns this list with the index position of all run bases.
